...
int main (int argc, char** argv)
{
	...
	if (mpi_rank != 0)
	{
		...
		int buff_size = d.nx * d.ny;
		// printf("[Process %d] buff_size: %i\t index_xy: %li\n",mpi_rank, buff_size, index(d.startx, d.starty, d.nx));
		MPI_Send(c, d.nx*d.ny, MPI_INT, 0, 111, p.comm);
		// printf("[Process %d] sent", mpi_rank);
	}
	/****************************************************************************/
	// Write the image
	if (mpi_rank == 0)
	{
		...
		for (int proc = 1; proc < mpi_size; proc++)
		{
			...
			int buff_size = d1.nx * d1.ny;
			// printf("[Process %d] buff_size: %i\t index_xy: %li\n",mpi_rank, buff_size, index(d1.startx, d1.starty, d1.nx));
			MPI_Recv(c, d1.nx*d1.ny, MPI_INT, proc, 111, p.comm, MPI_STATUS_IGNORE);
